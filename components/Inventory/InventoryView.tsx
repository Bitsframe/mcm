...
/**
 * Per-location stock. Reads the sidebar's location unless a `location` is
 * passed — the Warehouse page does that to show any location's stock.
 */
export const InventoryView = ({ location }: { location?: any }) => {
  ...
  const { selectedLocation: contextLocation } = useContext(LocationContext);
  const selectedLocation = location ?? contextLocation;
  ...
  const RightSideComponent = useMemo(
    () => (
      <div className="text-sm text-label-2 flex flex-col sm:flex-row items-start sm:items-center justify-end space-y-2 sm:space-y-0 sm:space-x-2">
        <div className="flex items-center space-x-0 bg-gray-100 rounded-md overflow-hidden">
          <button
            onClick={handleActiveClick}
            className={`flex items-center gap-x-1 px-3 sm:px-4 py-2 text-xs sm:text-sm ${
              !getDataArchiveType
                ? "bg-brand-600 text-white"
                : "bg-transparent text-label-2"
            }`}
          >
  ...
  const MobileRightSideComponent = useMemo(
    () => (
      <div className="flex items-center gap-2 w-full">
        <div className="flex-1 flex items-center space-x-0 bg-gray-100 rounded-md overflow-hidden">
          <button
            onClick={handleActiveClick}
    ...
    <main className="w-full h-full font-[500] text-[20px]">
      <div className="w-full overflow-auto py-2 px-2">
        ...
        <div className="h-[100%] col-span-2 rounded-md py-2">
          <div className=" pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            <div className="flex items-center gap-x-2 w-full sm:w-auto">
              <input
                ...
                className="px-3 py-2 w-full sm:w-64 text-sm rounded-md focus:outline-none border border-gray-300 bg-[#F5F5F7]"
              />
            </div>
          ...
          <div className="flex min-h-[60dvh] flex-col overflow-hidden rounded-lg border border-border shadow-mac-sm">
            {/* Desktop Table View */}
                                ...
                                <PiCaretUpDownBold
                                  className={`inline ${
                                    sortColumn === id
                                      ? "text-brand-600"
                                      : "text-gray-400"
                                  } hover:text-label-2 active:text-label-2`}
                                />
                              </button>
                            )}
                          </div>
                  ...
                  {loading ? (
                    ...
                    <TableRow className="flex" style={{ height: '400px' }}>
                      ...
                      <TableCell className="flex-1 flex items-center justify-center">
                        <h1 className="text-label-2">{t("Inventory_k45")}</h1>
                      </TableCell>
                    </TableRow>
                  ) : (
