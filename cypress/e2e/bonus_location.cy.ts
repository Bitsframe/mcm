...
// 12 focused tests covering all real functionality
...
describe("Bonus — Location Bonus Page", () => {
  ...
  // ── 1. Page load, tab navigation, table structure ──────────────────────────
  // Merged: page load + tab switching + headers + date filter into one smoke test
  ...
  it("should load page, switch all tabs, show correct headers, and update date label", () => {
    ...
    cy.log("[UI] Page loaded, Calculation tab active by default");
    ...
    cy.log("[UI] Yesterday date label updated");

    cy.get("table tbody tr", { timeout: 10000 }).should("have.length.greaterThan", 0);
  });

  // ── 2. Calculate button — RPC call, loading state, toast ───────────────────

  it("should click Calculate, show Calculating... state, call RPC, show success toast, and return to normal", () => {
    loginAndVisitBonus();
    ...
    cy.log("[UI] Calculate button returned to normal, toast shown");
  });

  // ── 3. Calculation correctness — FLAT and PERCENTAGE ───────────────────────
  // Both types verified in one test to avoid duplicate login/visit
  ...
  it("should verify FLAT and PERCENTAGE bonus amount calculations match formula for Yesterday", () => {
    ...
    cy.get("table tbody tr", { timeout: 15000 }).then(($rows) => {
      ...
      $rows.each((_, row) => {
        ...
        const valueRaw   = (cells[5]?.textContent || "").replace(/[$%,]/g, "").trim();
      ...
      if (!flatVerified) cy.log("[FLAT] No FLAT rows with full data for yesterday — skipping");
      if (!pctVerified) cy.log("[PCT] No PERCENTAGE rows with full data for yesterday — skipping");
    });
  });
  ...
  it("should open Filter sheet, apply date filter and name filter, verify table updates", () => {
    ...
    // Name filter — pick first location from table
    cy.get("table tbody tr", { timeout: 10000 }).should("have.length.greaterThan", 0);
  ...
  // ── 5. Set limits — FLAT save, DB verify, Calculation tab verify ───────────

  it("should save FLAT config in Set limits, confirm DB bonus_config_history, verify in Calculation tab", () => {
    loginAndVisitBonus();
    ...
    // Update button disabled before edit
    ...
    cy.get("table tbody tr").first().find("td").eq(0).invoke("text").then((locationName) => {
      ...
      cy.log(`[SET LIMITS] Editing: "${locName}"`);
      ...
      cy.wait("@saveConfig", { timeout: 15000 }).then((interception) => {
        cy.log(`[SET LIMITS] Save payload: ${JSON.stringify(interception.request.body)}`);
      });
  ...
  // ── 6. Set limits — PERCENTAGE save, DB verify, Calculation tab verify ──────

  it("should save PERCENTAGE config in Set limits, confirm DB bonus_config_history, verify in Calculation tab", () => {
    loginAndVisitBonus();
  ...
  // ── 7. Transactions tab — columns, paid rows, DB match ─────────────────────
  // Merged: column check + paid badge check + DB data match into one test

  it("should show all columns in Transactions tab, verify paid rows have Paid badge and paid_date, and match DB", () => {
    loginAndVisitBonus();
    ...
    cy.get("body").then(($body) => {
      ...
      // Every row: Paid badge in last column, non-empty paid_date in col 7
      cy.get("table tbody tr").each(($row) => {
        cy.wrap($row).find("td").last().invoke("text").then((s) => {
      ...
      cy.log("[UI] All rows have Paid badge and paid_date");
      ...
      cy.get("table tbody tr").first().find("td").then(($cells) => {
        ...
        cy.task("getAllLocations").then((locs) => {
          ...
          if (!match) { cy.log(`[DB] No location_id found for "${locationName}"`); return; }
          ...
          cy.task("getBonusRowsForLocation", { locationId: match.id, limit: 10 }).then((rows) => {
            ...
            if (!paidRow) { cy.log("[DB] No paid row found — data may be stale"); return; }
  ...
  // ── 8. Pay toggle — click Pay, API call, toast, DB paid=true, appears in Transactions ──
  ...
  it("should click Pay, call update-paid API, show toast, verify DB paid=true, and row appears in Transactions tab", () => {
    ...
    cy.get("table tbody tr", { timeout: 15000 }).then(($rows) => {
      ...
      if (payRowIndex < 0) {
        cy.log("No eligible Pay rows for Yesterday — trying Today");
        cy.contains("button", "Today").click({ force: true });
        cy.wait(1500);
        ...
        cy.get("table tbody tr", { timeout: 15000 }).then(($todayRows) => {
          ...
          if (payRowIndex < 0) {
            cy.log("No eligible Pay rows on either date — skipping (no bonus_eligibility=true data)");
            return;
          }
    ...
    function runPayToggle(payRowIndex: number) {
      cy.get("table tbody tr").eq(payRowIndex).find("td").eq(0).invoke("text").then((locName) => {
        ...
        cy.log(`[UI] Paying bonus for: "${locationName}"`);
        ...
        cy.wait("@updatePaid", { timeout: 15000 }).then((interception) => {
          ...
          // DB verify
          cy.wait(1000);
          cy.task("getBonusRowForLocationAndDate", { locationId: Number(locationId), date: String(date) })
            .then((bonusRow) => {
          ...
          cy.task("getBonusRowsForLocation", { locationId: Number(locationId), limit: 5 })
            .then((rows) => cy.log(`[DB] Latest 5 rows for location ${locationId}: ${JSON.stringify(rows)}`));
          ...
          cy.contains("td", locationName).closest("tr").find("td").eq(7)
            .invoke("text").then((d) => { expect(d.trim()).to.not.eq("-"); cy.log(`[UI] ✓ Paid date: "${d.trim()}"`); });
        });
      });
    }
  });

  // ── 7. Transactions tab — columns, paid rows, DB data match ────────────────
  ...
  // ── 9. Bonus Eligibility — badge, DB match, Pay button gating, Calculate sets it ──

  it("should show Yes/No badge matching DB, Pay button gated by eligibility, and Calculate sets eligibility=true", () => {
    loginAndVisitBonus();
    ...
    cy.get("table tbody tr").first().find("td").then(($cells) => {
      ...
      cy.task("getAllLocations").then((locs) => {
        ...
        const yesterday = getYesterdayYMD();
        cy.task("getBonusRowForLocationAndDate", { locationId: match.id, date: yesterday }).then((bonusRow) => {
          cy.log(`[DB] bonus row for location_id=${match.id}, date=${yesterday}: ${JSON.stringify(bonusRow)}`);
          if (!bonusRow) {
    ...
    // Run Calculate for Today and verify eligibility updates
    cy.contains("button", "Today").click({ force: true });
    cy.wait(800);
...
// ─── End-to-End: POS Sale → Set Threshold → Calculate → Bonus Eligibility ────

describe("Bonus — E2E: POS Sale to Bonus Eligibility", () => {
  ...
  it("should set threshold=$1, place POS sale, run Calculate, verify bonus_eligibility=true in DB and Yes badge in UI", () => {

    // STEP 1: Set threshold=$1 so any sale qualifies
    cy.log("=== STEP 1: Set bonus threshold to $1 in Set limits ===");
    cy.loginWithCredentials(TEST_EMAIL, TEST_PASSWORD);
    cy.wait(2000);
    cy.visit(BONUS_LOCATION_URL);
    cy.wait(2000);
