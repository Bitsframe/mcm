...
const List_Item = ({
  ...
  return (
    <div
      onClick={() => click_handle(data)}
      className={`${
        is_selected ? "bg-text_primary_color" : "bg-[#D9D9D9]"
      }   px-3 py-3 rounded-lg flex justify-between cursor-pointer pointer-events-auto`}
    >
      <div>
        <h1
          className={`${
            is_selected ? "text-white" : "text-text_primary_color"
          }  font-bold text-xl`}
        >
...
const Appoinments = () => {
  ...
  const [appointment_details, setAppointment_details] =
  ...
  const delete_appointments_handle = async (delId: number) => {
    ...
    if (!error) {
      setAppointments(
        appointments.filter((appoint: any) => appoint.id !== delId)
      );
      setAllAppointments(
        appointments.filter((appoint: any) => appoint.id !== delId)
      );
  ...
  const newAddedRow = (row: any) => {
    ...
    (async function getLocations() {
      const appoint_data: any = await fetchAppointmentsByLocation(
        selectedLocation.id
      );
      setAllAppointments(appoint_data);
      setAppointments(appoint_data);
  ...
  const format_date_time = (
    ...
    return moment(dateTimeString, "DD-MM-YYYY hh:mm A").format(
      "YYYY-MM-DD HH:mm:ss"
    );
  };
    ...
    if (column === "gender") {
      ...
      sortedList = appointments.sort((a, b) => {
        ...
        if (a.date_and_time) {
          ...
          const [datePart, timePart] = a.date_and_time
            .split("|")[1]
            .trim()
            .split(" - ");
        ...
        if (b.date_and_time) {
          const [datePart, timePart] = b.date_and_time
            .split("|")[1]
            .trim()
            .split(" - ");
        ...
        if (sortOrder === -1) {
          return (
            new Date(a_formatted_date).getTime() -
            new Date(b_formatted_date).getTime()
          );
        } else {
          return (
            new Date(b_formatted_date).getTime() -
          ...
          <div className="flex  items-center space-x-5">
            ...
            <div>
              ...
              <div className="flex text-sm items-center space-x-3">
                <button
                  onClick={() => sortHandle("gender")}
                  className="bg-text_primary_color px-2 py-1 rounded-md text-white active:opacity-60"
                >
                  Gender
                </button>
                <button
                  onClick={() => sortHandle("slot")}
                  className="bg-text_primary_color px-2 py-1 rounded-md text-white active:opacity-60"
                >
                  Appointment slot
                </button>
          ...
          {appoint_loading ? (
            ...
            appointments.map((appointment: Appointment, index: any) => (
              <List_Item
                is_selected={
                  appointment_details &&
                  appointment_details.id === appointment.id
                }
                click_handle={select_for_details_handle}
                key={index}
                data={appointment}
              />
            ))
          ...
          {appointment_details ? (
            <div className="flex flex-col h-full">
                  ...
                  return (
                    <h1 key={index}>
                      ...
                      <span className="font-normal">
                        {elem.date_format
                          ? Moment(appointment_details["created_at"]).format(
                              "LLL"
                            )
                          : elem.type === "date_slot" &&
                          ...
                          : appointment_details
                          ? appointment_details[key as keyof typeof Appoinments]
                          : "-"}
                      </span>
                    </h1>
                  );
              ...
              <div className="w-full flex mt-3 gap-3">
                <button
                  onClick={() =>
                    delete_appointments_handle(appointment_details.id)
                  }
                ...
                <Appointment_Edit_Modal
                  ...
                  location_data={
                    find_locations(appointment_details.location_id)!
                  }
                />
              </div>
            </div>
