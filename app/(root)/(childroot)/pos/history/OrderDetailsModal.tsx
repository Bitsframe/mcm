...
const calcTotalAmount = (list: any) => {
  const totalVal = list?.sales_history?.reduce(
...
const tableHeader = [
  // Same table headers as before
  {
    ...
    render_value: (val: string, elem: any) =>
      elem?.inventory?.products?.categories?.category_name,
  },
  {
    id: "product_name",
    label: "Product",
...
const PatientDetailsRender: FC<PatientDetailsRenderPropsInterface> = ({
  patientData,
  paymentType,
}) => {
  const {
  ...
  return (
    <div className="py-4 space-y-3">
      ...
      <div className="text-base grid grid-cols-3 gap-6">
        ...
        <p>
          Treatment Type: <strong>{treatmenttype}</strong>
        </p>
        <p>
          Location: <strong>{Locations?.title}</strong>
        </p>
      </div>
    </div>
  );
...
const ReturnProductSection = ({
  ...
  const processReturnHandle = async (e: React.FormEvent) => {
    ...
    if (forReturnQty > data.quantity_sold) {
      toast.error(
        `Return quantity should not be higher than the sold quantity`
      );
      return;
    }
    setLoading(true);
    ...
    const { data: resData, error } = await create_content_service({
      table: "returns",
      language: "",
      post_data: postData,
    });
  ...
  if (processReturn) {
    return (
      <div className=" fixed bg-black/40 top-0 bottom-0 left-0 right-0 flex justify-center items-center">
          ...
          <div>
            <form onSubmit={processReturnHandle} className="space-y-6">
              ...
              <div className="flex justify-start flex-col space-y-1">
                ...
                <select
                  onChange={changeReasonHandle}
                  className="border-2 text-sm rounded-md px-2 py-2 flex items-center space-x-2 "
                >
                  <option value={""} disabled selected>
                    Select Reason
                  </option>
                  {/* @ts-ignore */}
                  {preDefinedReasonList.map((opt, ind) => (
                    <option key={ind} value={opt?.reason}>
                      {opt?.reason}
                    </option>
                  ))}
                </select>
              ...
              <Button
                type="submit"
                disabled={
                  loading || !forReturnReason || !forReturnQty ? true : false
                }
  ...
  if (returnedQty && !loading) {
    return (
      <button
        className="border-[#E4E4E7] text-[#696969] border-2 text-xs px-3 py-3 rounded-md"
        disabled
      >
  ...
  return (
    <button
      disabled={isAnyReturned}
      onClick={returnHandle}
      className="bg-[#E1BBB8] text-sm px-3 py-3 rounded-md disabled:opacity-60"
    >
      Return
    </button>
  );
...
const TableRowRender: FC<TableListRenderInterface> = ({
  dataList,
  order_id,
  isAnyReturned,
  hasReturnedHandle,
  ...
  return (
    <div
      className={` flex items-center flex-1 text-base py-5 border-b-2 border-b-[#E4E4E7]`}
    >
      {tableHeader.map(({ id, render_value, align, flex }, ind) => {
        ...
        return (
          <h1
            key={ind}
            className={`${flex ? flex : "flex-[4]"} ${align || "text-center"}`}
          >
            {id === "actions" ? (
        ...
        <div className="bg-white rounded-lg w-3/4 p-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <p className="text-base">
              Sales Amount: <strong>{calcTotalAmount(dataList)}</strong>
            </p>
          </div>
