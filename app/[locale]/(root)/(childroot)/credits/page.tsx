    ...
    const fetchData = async () => {
      ...
      try {
        const creditsData: CreditData[] = await fetch_content_service({
          ...
          matchCase: [
            { key: "patientData.locationid", value: selectedLocation.id },
          ],
          filterOptions: [
            { operator: "not", column: "patientData", value: null },
          ],
        });
        setCredits(creditsData);
        const total = creditsData.reduce(
          (sum: number, credit: CreditData) => sum + credit.balance,
          0
        );
                    ...
                    >
                      ...
                      <TableCell className="text-xs sm:text-sm px-2 py-1 sm:px-4 sm:py-2">
                        <div className="line-clamp-1">
                          {credit.patientData?.firstname}{" "}
                          {credit.patientData?.lastname}
                        </div>
                      </TableCell>
                      <TableCell className="text-xs sm:text-sm px-2 py-1 sm:px-4 sm:py-2">
